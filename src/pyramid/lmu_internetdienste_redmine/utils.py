...
def auth_user(username, password, redmine=__redmine, url=None):
    user = None
    try:
        if url != None:
            user = Redmine(url, username=username, password=password, requests={'verify': False}).auth()
        else:
            user = Redmine(redmine.url, username=username, password=password, requests={'verify': False}).auth()
    except AuthError:
...
def setup_webproject(project_name,
    ...
    add_milestone_initialgespraech(project)
    add_milestone_playlanduenbergabe(project)
    add_milestone_onlinegang(project)
    add_milestone_initialgespraech(project)
    add_milestone_initialgespraech(project)
    add_milestone_pflege
    add_milestone_endoflive(project)
